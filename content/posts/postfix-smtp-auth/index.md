...
比較好的作法可能是使用 VPN 先連到內網後再來寄信。不過，還有一個更方便的方式，也就是這篇想要介紹的 SMTP AUTH(authentication)。SMTP AUTH 允許我們先透過使用者名稱、密碼進行認證，如果認證通過後就能夠用該 SMTP 來寄信。
